...
if [ "`uname`" == 'Darwin' ]; then
	...
	echo "writing environment variables to ~/.pam_environment"
	echo "AWS_ACCESS_KEY_ID=AKIAJ7JRXK43OVKXUM7A" >> ~/.pam_environment
	echo "AWS_SECRET_ACCESS_KEY=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" >> ~/.pam_environment
	echo "AWS_STORAGE_BUCKET_NAME=pacific_energy" >> ~/.pam_environment
	echo "JOBLADDER_GIT=https://serempre.repositoryhosting.com/git/serempre/jobladder.git" >> ~/.pam_environment
	echo "USE_FOREMAN=True" >> ~/.pam_environment
	echo "fixed environment variables! you are ready to use AWS S3 bucket for static & media files"
	#ask user if database credentials are required
	USE_DB_CREDENTIALS='wtf';
	while [[ "$USE_DB_CREDENTIALS" != "yes" && "$USE_DB_CREDENTIALS" != "no" ]]; do 
	...
	#set database credentials environment variable
	if [ "$USE_DB_CREDENTIALS" == "yes" ]; then
		echo "USE_POSGRES_USER=True" >> ~/.pam_environment
	else
		echo "Database connections will won't pass user credentials when negotiating a connection"
	fi
fi
